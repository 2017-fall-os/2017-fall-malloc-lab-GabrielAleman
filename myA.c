...
void *bestFit(size_t s){
  ...
  int bestPSpace = 0;
	...
	if (!p->allocated && computeUsableSpace(p) == s)
	  ...
	  if(bestPSpace == 0){
	    bestPSpace = computeUsableSpace(p);
	    bestP = p;
	  }
	  else if(bestPSpace > computeUsableSpace(p)){
	    bestPSpace = computeUsableSpace(p);
	    bestP = p;
	  }
	}
...
void *resizeRegion(void *r, size_t newSize) {
  int oldSize;
  BlockPrefix_t *nextP;
  BlockPrefix_t *currentP = regionToPrefix(r);
  if (r != (void *)0)		/* old region existed */
    oldSize = computeUsableSpace(currentP);
  else
    oldSize = 0;		/* non-existant regions have size 0 */
  if (oldSize >= newSize)	/* old region is big enough */
    return r;
  else{
    nextP = getNextPrefix(regionToPrefix(r));
    if(!nextP->allocated && (computeUsableSpace(nextP) + computeUsableSpace(currentP)) >= newSize){
      currentP->allocated = 0;
      currentP = coalescePrev(nextP);
      currentP->allocated = 1;
      return prefixToRegion(currentP);
    }
  }
